...
int getExpense(int depth, int color) {
	if (depth == (N - 1)) {
		if (color == 0) { return payList[N - 1].r; }
		if (color == 1) { return payList[N - 1].g; }
		if (color == 2) { return payList[N - 1].b; }
	}
...
int main() {
	...
	cout << min(min(getExpense(0, 0), getExpense(0, 1)), getExpense(0, 2)) << endl;
